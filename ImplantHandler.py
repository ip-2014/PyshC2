...
import os, time, readline, base64, re, traceback, glob
import datetime
import time
...
def startup(printhelp = ""):
  ...
  try:
    ...
    if (implant_id.lower() == "automigrate-frompowershell") or (implant_id.lower() == "am"):
      startup("automigrate not currently implemented for PyshC2\r\n")
    ...
    if "set-clockworksmsnumber" in implant_id.lower():
      ...
      startup("Updated set-clockworksmsnumber: %s\r\n" % cmd)
...
def commandloop(implant_id):
  while(True):
    ...
    try:
      ...
      print "Error running against the selected implant ID, ensure you have typed the correct information: %s" % e
      traceback.print_exc()
      print "Error: %s" % e
